...
func QueryAllReplyNotify(cg *config.Config, username string, limit int32, offset int32) ([]int32, error) {
	res := make([]int32, 0)
	if err := cg.Tx.WithContext(cg.Ctx).Model(&ReplyNotify{}).Select("id").Where("username = ?", username).Where("isdelete = ?", false).Order("updatedAt DESC").Limit(int(limit)).Offset(int(offset)).Find(&res).Error; err != nil {
